...
function deleteMySocketFromMatching(socket, io, roomName) {
  // 14) 소켓 룸에서 제거
  logger.info("Removing socket from room", `socketId:${socket.id}, roomName:${roomName}`);
  ...
  if (room) {
    logger.debug("Found room, updating priority trees for all sockets", `roomName:${roomName}, socketCount:${room.size}`);
    ...
    room.forEach((socketId) => {
      ...
      if (roomSocket) {
        ...
        logger.debug("Removed socket from roomSocket's priority tree", `roomSocketId:${roomSocket.id}, memberId:${roomSocket.memberId}`);
        logger.debug(
    ...
    logger.warn(`Room ${roomName} does not exist or is empty`, `socketId:${socket.id}`);
  }

  // 16) 나의 priorityTree 삭제
  logger.info("Clearing priority tree and resetting highestPriorityNode", `socketId:${socket.id}, memberId:${socket.memberId}`);
